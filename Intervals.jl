...
struct Interval
    ...
    function Interval(lbound::AbstractVector{Float64}, ubound::AbstractVector{Float64})
        return any(lbound .> ubound) ? error("out of order") : new(lbound, ubound)
    end
end
...
function draw_spreads(
    ...
    if spread_min > spread_max
        throw(ArgumentError("the condition `spread_min <= spread_max` is not satisfied"))
    end
...
function draw_centers(rng::AbstractRNG, spread::Real; x_min=X_MIN, x_max=X_MAX)
    ...
    elseif lbound >= ubound
        throw(
            ArgumentError("the condition `spread <= (x_min - x_max) / 2` is not satisfied"),
        )
    else
...
function draw_centers(rng::AbstractRNG, spread::Vector; x_min=X_MIN, x_max=X_MAX)
...
function draw_interval(
    ...
    width_min = max(spread_min * 2, volume_min / volume(; centers=centers, spreads=spreads))
    ...
    while width_min > width_max && iter < iter_max
        iter += 1
        println(
            "Rejecting due to min width greater max width " * "($width_min > $width_max).",
        )
...
function plot_interval(ax, interval; x_min=X_MIN, x_max=X_MAX, score=nothing, kwargs...)
    ...
    rectangle =
        plt.Rectangle((x1_start, x2_start), x1_end - x1_start, x2_end - x2_start; kwargs...)
